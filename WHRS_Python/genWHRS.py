...
seed=2480+2          # Seed for random values
...
def betterOutput(tA,tB):
    ...
    return 0 # Can not decice

def paramDist(RG,interval):
    return RG.uniform(interval[0],interval[1])

def genRandomInput(RG,prange):
    Load         =paramDist(RG,prange['Load'])
    JW_pump      =paramDist(RG,prange['JW_pump'])
    RC_Superheat =paramDist(RG,prange['RC_Superheat'])
    RC_Subcool   =paramDist(RG,prange['RC_Subcool'])
    ORC_Superheat=paramDist(RG,prange['ORC_Superheat'])
    ORC_Subcool  =paramDist(RG,prange['ORC_Subcool'])
    ORC_Pump     =paramDist(RG,prange['ORC_Pump'])
    P_chamber    =paramDist(RG,prange['P_chamber'])
...
for it in range(NO):
    (Load,JW_pump,
     RC_Superheat,RC_Subcool,
     ORC_Superheat,ORC_Subcool,ORC_Pump,P_chamber)=genRandomInput(RG,rs.params_range)
    t=rs(Load,JW_pump,
          RC_Superheat,RC_Subcool,
          ORC_Superheat,ORC_Subcool,ORC_Pump,P_chamber)
...
#%% Write to csv files
header=['Exergy_eff_WHRS','CO2_red','EPC','Exergy_eff_WHRS','CO2_red','EPC']
